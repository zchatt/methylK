#!/bin/bash
# count cg content in uniquely pseudoaligned reads
...
#awk '{print $2}' $OUTDIR/${ASSAY}_counts2 | sort | uniq -c | awk '{ print $2 " " $1}' | sed 1d | sort -n > $OUTDIR/${ASSAY}_c_occurance
# count occurances of G ($3)
awk '{print $3}' $OUTDIR/${ASSAY}_counts2 | sort | uniq -c | awk '{ print $2 " " $1}' | sort -n > $OUTDIR/${ASSAY}_g_occurance
#awk '{print $3}' $OUTDIR/${ASSAY}_counts2 | sort | uniq -c | awk '{ print $2 " " $1}' | sed 1d | sort -n > $OUTDIR/${ASSAY}_g_occurance
...
#awk -v a="$(grep $ASSAY_TOTAL $OUTDIR/total_assigned_assay | awk '{print $2}')" '{print $1 " " $2/a " " $3/a}' $OUTDIR/${ASSAY}_cgcounttemp > $OUTDIR/${ASSAY}_proptemp
rm $OUTDIR/${ASSAY}_counts2 $OUTDIR/${ASSAY}_c_occurance $OUTDIR/${ASSAY}_g_occurance $OUTDIR/${ASSAY}_tempC* $OUTDIR/${ASSAY}_tempG* 2> /dev/null
}
export -f count_cg
...
chars=$(head -1000 $OUTDIR/tmp5 | awk '{print $3}' | wc -c)
words=$(head -1000 $OUTDIR/tmp5 | awk '{print $3}' | wc -w)
...
paste $OUTDIR/*cgcounttemp.new > $OUTDIR/${SAMPLE_NAME}_cgcount

# remove temp count files
